...
public class Pub2AgentsArgs extends BasicArgs {
	...
	@Parameter(names = { "-get-bioagents" }, description = "Fetch the entire current bio.agents content using the bio.agents API to the file bioagents.json in the given output directory")
	String getBioagents = null;

	@Parameter(names = { "-copy-bioagents" }, description = "Copy the file containing bio.agents content in JSON format given with the --bioagents parameter to the file bioagents.json in the given output directory")
	String copyBioagents = null;

	@Parameter(names = { "-select-pub" }, description = "Fetch publication IDs of journal articles from the given period (given with --from/--to or --month or --day) that are potentially suitable for bio.agents to the file pub.txt in the given output directory")
	...
	@Parameter(names = { "-pass1" }, description = "The first pass of the Pub2Agents algorithm will load all publications from db.db corresponding to the publication IDs in pub.txt and write found agent names with scores and other data to pass1.json, with related links extracted from publication abstracts and fulltexts written to web.txt and doc.txt in the given output directory")
	String pass1 = null;
	...
	@Parameter(names = { "-pass2" }, description = "The second pass of the Pub2Agents algorithm will load all results of the first pass from pass1.json and after iterating over these, while processing them and calculating new results, it will write the final results to results.csv, new.json and diff.csv in the given output directory")
	String pass2 = null;

	@Parameter(names = { "-map", "-edammap" }, description = "This step will add EDAM ontology annotations using EDAMmap to the Pub2Agents results in new.json, outputting the annotated results to to_bioagents.json in the given output directory")
	String map = null;

	@Parameter(names = { "-all" }, description = "This command will run all setup commands necessary for getting the files required by the steps and then run all steps in order (-fetch-pub, -pass1, -fetch-web, -pass2, -map). Required arguments for the setup commands are --edam, --idf, --idf-stemmed and either --from/--to or --month or --day or --pub. Arguments --bioagents and --db can also be specified.")
	String all = null;
	...
	@Parameter(names = { "--bioagents", "--bioagents-json" }, description = "The JSON file containing the entire bio.agents content to be copied to the output directory with -copy-bioagents (or -all)")
	String bioagents = null;
	...
	@Parameter(names = { "--disable-agent-restriction" }, description = "If specified, using phrase combinations to narrow down publication IDs to only those potentially about agents is not done with -select-pub (or -all)")
	boolean disableAgentRestriction = false;
	...
	@Parameter(names = { "-before-after" }, description = "Generate a list of words that should increase the score of a suggestion, when one of these words occurs before or after that suggestion. Words that should increase the score more are output more in front. The presumable suggestion is taken from the agent_title, if one exists. Required args: --idf, --pub, --db.")
	boolean beforeAfter = false;

	@Parameter(names = { "-europepmc-abstract" }, description = "Calculate how frequent a token is in an abstract about a agent vs in all possible abstracts from Europe PMC (for all tokens that occur at least the given number of times). Required args: --db (containing all publications of bio.agents).")
