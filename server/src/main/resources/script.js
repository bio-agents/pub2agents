...
var bioagentsExisting = function(text, arr) {
	if (arr != null && arr.length > 0) {
		return '<span>' + text + ' ' + arr.map(e => '<a href="https://bio.agents/' + escape(e.split(' (')[0]) + '">' + escape(e.split(' (')[0]) + '</a>').join(', ') + '</span><br>';
	} else {
		return '';
	}
};
...
var run = function(id, endpoint) {
	...
	document.getElementById('pub2agents-results').readOnly = true;
	...
	request.onreadystatechange = function() {
		if (request.readyState == 4 && request.status > 0) {
			...
			if (json.success) {
				...
				if (id == 'withoutmap' || id == 'all') {
					if (id == 'all') {
						...
						delete jsonWithoutMap['agent']['function'];
						delete jsonWithoutMap['agent']['topic'];
						document.getElementById('pub2agents-results').value = JSON.stringify(jsonWithoutMap['agent'], null, 2);
					} else {
						document.getElementById('pub2agents-results').value = JSON.stringify(json['agent'], null, 2);
					}
					if (json['agent']['confidence_flag'] == 'high') {
						document.getElementById('pub2agents-results').classList.add('input-good');
					} else if (json['agent']['confidence_flag'] == 'medium') {
						document.getElementById('pub2agents-results').classList.add('input-medium');
					} else {
						document.getElementById('pub2agents-results').classList.add('input-bad');
					}
					if (json['status']) {
						var output_html = '';
						if (!json['status']['include']) {
							output_html += '<span>Not a agent!</span><br>';
						}
						...
						output_html += bioagentsExisting('Existing in bio.agents as', json['status']['existing']);
						output_html += bioagentsExisting('Same publications and name as in', json['status']['publicationAndNameExisting']);
						output_html += bioagentsExisting('Same name and some publications in common with', json['status']['nameExistingSomePublicationDifferent']);
						output_html += bioagentsExisting('Some publications in common but name different from', json['status']['somePublicationExistingNameDifferent']);
						output_html += bioagentsExisting('Same name but publications different in', json['status']['nameExistingPublicationDifferent']);
						output_html += bioagentsExisting('Name similar to', json['status']['nameMatch']);
						...
						if (json['status']['agentsExtra'] != null && json['status']['agentsExtra'].length > 0) {
							output_html += '<span>Given publications could contain extra agents: ' + json['status']['agentsExtra'].map(e => '"' + escape(e) + '"').join(', ') + '</span><br>';
						}
						document.getElementById('pub2agents-results-output').innerHTML = output_html;
					}
				}
				if (id == 'map' || id == 'all') {
					document.getElementById('to-bioagents-output').innerHTML = escape(JSON.stringify(json['agent'], null, 2));
					document.getElementById('to-bioagents-output').style.height = (document.getElementById('to-bioagents-output').scrollHeight + 4) + 'px';
				}
			...
			document.getElementById('pub2agents-results').readOnly = false;
	...
	if (id == 'withoutmap' || id == 'all') {
		...
		if (document.getElementById('pub2agents-results').value != '') {
			try {
				params['agent'] = JSON.parse(document.getElementById('pub2agents-results').value);
			} catch (error) {
	...
	if (id == 'withoutmap' || id == 'all') {
		document.getElementById('pub2agents-results').value = '';
		document.getElementById('pub2agents-results').classList.remove('input-good', 'input-medium', 'input-bad');
		document.getElementById('pub2agents-results-output').innerHTML = '';
	}
	document.getElementById('to-bioagents-output').innerHTML = '';
	document.getElementById('to-bioagents-output').style.height = '42px';
